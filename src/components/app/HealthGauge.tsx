...
export function HealthGauge({ hf, ltvBps, maxLtvBps, liqThresholdBps }: { hf?: bigint; ltvBps?: bigint; maxLtvBps: number; liqThresholdBps: number }) {
  // An unread position and a position with no debt are different facts. Treating
  // the first as infinite health draws a full, confident sweep for a number
  // nobody has: the gauge shows its empty track instead.
  const unknown = hf === undefined;
  const inf = !unknown && hf >= 2n ** 200n;
  const h = unknown ? 0.5 : inf ? 3 : Math.min(3, Number(hf) / 1e18);
  ...
  const tone = unknown ? "#a8a29e" : inf ? "#57534e" : h < 1 ? "#823a24" : h < 1.2 ? "#d97645" : "#292524";
